package ahmedt.m_arsipku.MemoTerkirim.CariMemoTerkirim;
...
import ahmedt.m_arsipku.MemoTerkirim.DataItem;
import ahmedt.m_arsipku.MemoTerkirim.MemoTerkirimModel;
import ahmedt.m_arsipku.R;
import okhttp3.Response;


public class CariMemoTerkirimActivity extends AppCompatActivity{
    ...
    private CariMemoTerkirimAdapter mAdapter;
    private static final String TAG = "CariMemoMasukActivity";
    ...
    private void findViews() {
        ...
        mAdapter = new CariMemoTerkirimAdapter(CariMemoTerkirimActivity.this, modelList);
        recyclerView.setHasFixedSize(true);
        // use a linear layout manager
        LinearLayoutManager layoutManager = new LinearLayoutManager(this);
        recyclerView.setLayoutManager(layoutManager);
        recyclerView.setAdapter(mAdapter);
        mAdapter.SetOnItemClickListener(new CariMemoTerkirimAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position, DataItem model) {
                //handle item click events here
                Intent i = new Intent(CariMemoTerkirimActivity.this, DetailActivity.class);
                i.putExtra("param", "memo_terkirim");
    ...
    private void setAdapter(String id_so, String key) {
        ...
        AndroidNetworking.post(Server.getURL_MemoTerkirim)
                .addBodyParameter("id_so", id_so)
                .addBodyParameter("page", "1")
                .addBodyParameter("key", key)
                .setTag("getMemoTerkirim")
                .build()
                .getAsOkHttpResponseAndObject(MemoTerkirimModel.class, new OkHttpResponseAndParsedRequestListener<MemoTerkirimModel>() {
                    @Override
                    public void onResponse(Response okHttpResponse, MemoTerkirimModel response) {
                        if (okHttpResponse.isSuccessful()){
                            ...
                            if (response.getCode()==200){
                                ...
                                for (int i = 0; i < response.getData().size(); i++) {
                                    ...
                                    items.setAlamatCabang(response.getData().get(i).getAlamatCabang());
                                    items.setAlamatSurat(response.getData().get(i).getAlamatSurat());
                                    ...
                                    items.setTtdPengirim(response.getData().get(i).getTtdPengirim());
                                    ...
                                    items.setPenulis(response.getData().get(i).getPenulis());
                                    items.setNamaPengirim(response.getData().get(i).getNamaPengirim());
                                    items.setLogoSurat(response.getData().get(i).getLogoSurat());
                                    items.setNamaCabang(response.getData().get(i).getNamaCabang());
                                    items.setJabatanTembusan(response.getData().get(i).getJabatanTembusan());
                                    items.setStatusSent(response.getData().get(i).getStatusSent());
                                    items.setJabatanPengirim(response.getData().get(i).getJabatanPengirim());
                                    items.setJabatanTertuju(response.getData().get(i).getJabatanTertuju());
                                    items.setArsip(response.getData().get(i).getArsip());
                                    items.setRahasia(response.getData().get(i).getRahasia());
                                    items.setIdKonsep(response.getData().get(i).getIdKonsep());
                                    items.setFootName(response.getData().get(i).getFootName());
                                    items.setRevisikonsep(response.getData().get(i).getRevisikonsep());
                                    items.setJenisNd(response.getData().get(i).getJenisNd());

                                    items.setFileMemo(response.getData().get(i).getFileMemo());
                                    items.setKonsep(response.getData().get(i).getKonsep());
                                    modelList.add(items);
                                }
        ...
        AndroidNetworking.post(Server.getURL_MemoTerkirim)
                ...
                .getAsOkHttpResponseAndObject(MemoTerkirimModel.class, new OkHttpResponseAndParsedRequestListener<MemoTerkirimModel>() {
                    ...
                    public void onError(ANError anError) {
                        ...
                        btn_loadMore.setVisibility(View.VISIBLE);
                        Toast.makeText(CariMemoTerkirimActivity.this, "Jaringan atau Server Bermasalah", Toast.LENGTH_SHORT).show();
                    }
                });
    }
